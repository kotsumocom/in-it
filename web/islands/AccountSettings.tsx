...
export default function AccountSettings({
  ...
  const handleEmailChange = async (e: Event) => {
    e.preventDefault();
    if (!accessToken) {
      setMessage({ type: "error", text: "認証が忁E��でぁE });
      return;
    }
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "確認メールを送信しました。新しいメールアドレスのリンクをクリチE��して変更を完亁E��てください、E,
      });
      setNewEmail("");
  ...
  const handlePasswordChange = async (e: Event) => {
    ...
    if (!newPassword || !confirmPassword) {
      setMessage({ type: "error", text: "すべての頁E��を�E力してください" });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "パスワード�E変更に失敗しました");
      }
  ...
  return (
    <section class="p-6 bg-white border border-gray-200">
      <h2 class="text-lg font-bold text-gray-900 mb-4">アカウント設宁E/h2>
